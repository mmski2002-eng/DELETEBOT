Project / Protocol / Bridge Name:
Pharos Network / Pharos Mainnet

Blockchain / Network / Ecosystem:
Pharos Mainnet, an EVM-compatible Layer 1 blockchain focused on RealFi, real-world assets, stablecoins, institutional-grade assets, cross-chain liquidity, and high-performance parallel execution.

Protocol Type:
EVM-compatible Layer 1 blockchain; modular full-stack parallel L1; RealFi / RWA-focused infrastructure; cross-chain and cross-SPN interoperability network.

Audit Scope:
Pharos Mainnet architecture, consensus/execution assumptions, EVM compatibility layer, WASM/dual-VM behavior if applicable, SPN integrations, cross-chain communication, bridge/message flows, RPC behavior, explorer/indexer consistency, gas model, asset representations, stablecoin/RWA flows, compliance-related modules such as zk-KYC/AML if in scope.

Out of Scope:
Unknown unless specified. Exclude live destructive testing, unauthorized mainnet actions, validator disruption, real-user funds, and non-public infrastructure unless explicitly authorized.

Known roles:
Users, validators, RPC providers, indexers, explorers, developers, dApps, possible SPN operators, relayers/bridges, asset issuers, compliance modules, governance/admin/operator roles if present.

Known assumptions:
Pharos presents itself as an EVM-compatible Layer 1 for RealFi, RWA, stablecoins, cross-chain liquidity, modular architecture, deep-parallel execution, sub-second finality, AsyncBFT consensus, dual VM support, and cross-SPN/cross-chain communication.

Ты — исследователь логики сложной системы в рамках разрешённого аудита. Исследуешь блокчейн pharos mainet.  
...
Ограничения:
- Работай только в контексте легитимного анализа, аудита, тестнета, fork-тестов, локальной симуляции или read-only проверки.
- Не пиши destructive exploit-код.
- Не давай инструкций по атаке реальной сторонней системы.
- Высокоуровневые attack stories, тестовые сценарии, инварианты и безопасные проверки разрешены.
- Не называй идею известным классом уязвимости, пока не восстановишь её из логики самой системы.
