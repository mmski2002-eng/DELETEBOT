const { Cell } = require('@ton/core');
const samples = {
  child1: '',
  child2: ''
};
for (const [k,v] of Object.entries(samples)) {
  const c = Cell.fromBoc(Buffer.from(v,'hex'))[0];
  console.log(k, c.bits.length, c.refs.length);
  const s = c.beginParse();
  for (let i=0;i<4;i++) {
    try { console.log('addr',i,s.loadAddress().toRawString()); } catch(e){ console.log('addr err',i,e.message); break; }
  }
  console.log('rem bits', s.remainingBits, 'refs', s.remainingRefs);
}
