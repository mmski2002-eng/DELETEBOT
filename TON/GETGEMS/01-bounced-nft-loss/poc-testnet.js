...
function buildSaleDataCell(opts) {
  ...
  return beginCell()
    ...
    .storeAddress(opts.marketplaceAddress)                     // marketplace_address
...
async function main() {
  ...
  try {
    ...
    try {
      const result = await withRetry(
        () => client.runMethod(saleAddress, 'get_fix_price_data_v4'),
        'runMethod-get_fix_price_data_v4'
      );
      isComplete = result.stack.readNumber();
      // getter returns `is_complete == 1` → TVM true=-1, false=0
      console.log(`     is_complete: ${isComplete}${isComplete !== 0 ? ' ⚠️ ЗАВЕРШЕНА → NFT ПОТЕРЯН' : ''}`);
    } catch (e) {
  ...
  const attackerLostOverGwei = attackerDelta >= FULL_PRICE; // net loss ≈ full_price (mode=128 returns remainder)
  const saleCompleted = isComplete !== 0; // getter returns TVM bool: -1=true, 0=false
